...
(function () {
	...
	function setPage2( page ) {
		...
		Z.loop( 4, function ( i ) {
			...
			img.src = "img/page2-" + (i > 4 ? i - 1 : i) + ".png";

			img.wheight = Height * 0.3;
			...
			css( img, {
				...
				"-webkit-transform" : "translate3d(-50%," + (0 * offsetH + y) + "px,0) scale(" + (y / 400 + 1) + ")"
	...
	pages[2].onRemove = function () {
		...
		var offsetH = pages[1].offsetHeight * 0.2232;
		...
		Z.loopArray( lines, function ( line ) {
			...
			Z.css( line, {
				...
				"-webkit-transform" : "translate3d(-50%," + (50 * offsetH + y) + "px,0) scale(" + (y / 500 + 1) + ")",
	...
	// Z.onTap( pages[12].querySelector( ".page5-map" ), openMapPage );
	...
	// Z.onTap( document.querySelector( ".page23-btn" ), function () {
